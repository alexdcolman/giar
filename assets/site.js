...
(() => {
  ...
  style.textContent=`@media(max-width:700px){
    ...
    .graph-stage.mobile-selection-mode .graph-person-label-layer,.graph-stage.mobile-selection-mode .graph-person-lines{display:none!important}
  }`;
  ...
  const setup=()=>{
    ...
    if(cy.__GIAR_REV69_MOBILE__)return;
    cy.__GIAR_REV69_MOBILE__=true;
    ...
    cy.style()
      ...
      .selector('node[type = "person"].mobile-selection-native.label-visible.is-related').style({'text-opacity':1,'text-background-opacity':1,'text-background-padding':4,'text-border-width':1,'z-index':100})
      .selector('node[type = "person"].mobile-selection-native.label-visible.is-selected').style({'text-opacity':1,'text-background-opacity':1,'text-background-padding':4,'text-border-width':1,'z-index':120})
    ...
    const clearOverlay=()=>{for(const [,el] of labelEls)el.remove();labelEls.clear();lines.replaceChildren()};
    const syncMode=()=>{const on=hasSelection();stage.classList.toggle('mobile-selection-mode',on);cy.nodes('[type = "person"]').toggleClass('mobile-selection-native',on);if(on)clearOverlay();return on};
    const enterSelectionModeNow=()=>{stage.classList.add('mobile-selection-mode');cy.nodes('[type = "person"]').addClass('mobile-selection-native');clearOverlay()};

    const fitBodies=()=>{const nodes=visibleNodes();if(!nodes.length)return;cy.stop(true,false);cy.resize();const bb=nodes.boundingBox({includeLabels:false}),w=container.clientWidth,h=container.clientHeight,pad=8,fitZoom=Math.min((w-pad*2)/Math.max(1,bb.w),(h-pad*2)/Math.max(1,bb.h)),z=Math.max(cy.minZoom(),Math.min(cy.maxZoom(),fitZoom*.84)),cx=(bb.x1+bb.x2)/2,cy0=(bb.y1+bb.y2)/2;cy.zoom(z);cy.pan({x:w/2-cx*z,y:h/2-cy0*z})};

    let lastFocused='';
    const focusSelection=()=>{const sel=selectedNode();if(!sel.length){lastFocused='';return false}const nodes=cy.nodes('.is-selected, .is-related').filter(n=>!n.hasClass('is-hidden'));if(!nodes.length)return false;cy.stop(true,false);cy.resize();const vp=cy.getFitViewport?cy.getFitViewport(nodes,32):null;if(vp)cy.animate({zoom:vp.zoom,pan:vp.pan},{duration:220,easing:'ease-in-out-cubic',queue:false});else cy.fit(nodes,32);lastFocused=sel.id();return true};
    ...
    const selectPerson=id=>{const n=cy.getElementById(id);if(!n||!n.length||n.hasClass('is-hidden'))return;enterSelectionModeNow();n.emit('tap');requestAnimationFrame(()=>{syncMode();focusIfChanged();renderPeople()})};
    const ensureLabel=n=>{let el=labelEls.get(n.id());if(!el){el=document.createElement('button');el.type='button';el.className='graph-person-mobile-label';el.dataset.personId=n.id();el.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();selectPerson(el.dataset.personId)});labels.appendChild(el);labelEls.set(n.id(),el)}el.textContent=String(n.data('label')||'');el.classList.toggle('is-former',String(n.data('subtype')||'').toLowerCase().startsWith('ex '));return el};

    function renderPeople(){
      if(syncMode())return;
      const people=visibleNodes().filter('[type = "person"]').toArray(),keep=new Set(people.map(n=>n.id()));
    ...
    const depth=document.getElementById('selection-depth');if(depth)depth.addEventListener('change',()=>setTimeout(()=>{syncMode();focusSelection();renderPeople()},30));
    cy.on('tap',evt=>{if(evt.target!==cy&&evt.target.isNode?.())enterSelectionModeNow();requestAnimationFrame(()=>{syncMode();focusIfChanged();renderPeople()})});
